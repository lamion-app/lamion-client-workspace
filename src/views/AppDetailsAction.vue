  ...
  <article>
    <md-card class="content" v-if="loaded">
      ...
      <md-card-content>
        ...
        <md-list>
          <md-list-item
            >Дата создания <code>{{ app.date }}</code></md-list-item
          >
          <md-list-item
            >Rоличество событий <code>{{ app.eventsCount }}</code></md-list-item
          >
          <md-list-item
            >Общее количество запросов
            <code>{{ app.requestsCount }}</code></md-list-item
          >
        </md-list>
      </md-card-content>
      ...
      <md-card-content>
        <md-table md-sort="title" md-sort-order="asc" v-model="searched">
          <md-table-toolbar>
          ...
          >
            <md-table-cell md-label="Название" md-sort-by="title">{{
              item.title
            }}</md-table-cell>
            <md-table-cell
              md-label="Количество запросов"
              md-sort-by="requestsCount"
              >{{ item.requestsCount }}</md-table-cell
            >
          </md-table-row>
    ...
    <md-dialog-confirm
      ...
      v-model:md-active="dialogs.deleteApp"
      v-on:md-confirm="deleteApp"
    />

    <md-dialog class="app-edit-dialog" v-model:md-active="dialogs.editApp">
      ...
      <md-dialog-content>
        ...
        <md-field v-bind:class="getValidationClass('description')">
          ...
          <span class="md-error" v-if="!$v.form.title.maxLength"
      ...
      <md-dialog-actions>
        <md-button
          class="md-primary"
          v-bind:disabled="sending"
          v-on:click="dialogs.editApp = false"
          >Закрыть</md-button
        >
        <md-button
          class="md-primary"
